...
def create_player(state, player_type, player_id):
    ...
    if player_type == player.HUMAN_PLAYER:
        ...
        from .pvmcts import gomoku_pvmcts_torch_nn_player
        p = gomoku_pvmcts_torch_nn_player.create_player(player_id, state, 1000)
    elif player_type == player.GMCC_TORCH_NN_PLAYER:
        from .gmcc import gomoku_gmcc_torch_nn_player
        p = gomoku_gmcc_torch_nn_player.create_player(player_id, state)
    else:
        assert False
    return p
