...
from ..model.torch import gomoku_torch_network
from . import pvmcts_player

def create_model(state):
    return pvmcts_torch_nn_model.PVMctsTorchNNModel(state.get_name(), gomoku_torch_network.PVNetwork(state.get_state_numpy_shape(), state.get_action_dim()))
