...
def test_tictactoe_game_regression_mcts():
    assert run(['python', '-m', 'board_game.games.tictactoe_game_regression', 'random', 'random', '2'])

def test_gomoku_game_regression_mcts():
    assert run(['python', '-m', 'board_game.games.gomoku_game_regression', 'random', 'random', '2'])

def test_othello_game_regression_mcts():
    assert run(['python', '-m', 'board_game.games.othello_game_regression', 'random', 'random', '2'])

def test_cchess_game_regression_mcts():
    assert run(['python', '-m', 'board_game.games.cchess_game_regression', 'random', 'random', '2'])

def test_tictactoe_gmcc_table_train():
    assert run(['python', '-m', 'board_game.players.gmcc.tictactoe_gmcc_table_train', '--iteration_num', '2'])

def test_tictactoe_gmcc_torch_nn_train():
    assert run(['python', '-m', 'board_game.players.gmcc.tictactoe_gmcc_torch_nn_train', '--iteration_num', '2'])

def test_gomoku_gmcc_torch_nn_train():
    assert run(['python', '-m', 'board_game.players.gmcc.gomoku_gmcc_torch_nn_train', '--iteration_num', '2'])

def test_othello_gmcc_torch_nn_train():
    assert run(['python', '-m', 'board_game.players.gmcc.othello_gmcc_torch_nn_train', '--iteration_num', '2'])

def test_cchess_gmcc_torch_nn_train():
    assert run(['python', '-m', 'board_game.players.gmcc.cchess_gmcc_torch_nn_train', '--iteration_num', '2'])

def test_tictactoe_pvmcts_table_train():
    assert run(['python', '-m', 'board_game.players.pvmcts.tictactoe_pvmcts_table_train', '--iteration_num', '2'])

def test_tictactoe_pvmcts_torch_nn_train():
    assert run(['python', '-m', 'board_game.players.pvmcts.tictactoe_pvmcts_torch_nn_train', '--iteration_num', '2'])

def test_gomoku_pvmcts_torch_nn_train():
    assert run(['python', '-m', 'board_game.players.pvmcts.gomoku_pvmcts_torch_nn_train', '--iteration_num', '2'])

def test_othello_pvmcts_torch_nn_train():
    assert run(['python', '-m', 'board_game.players.pvmcts.othello_pvmcts_torch_nn_train', '--iteration_num', '2'])

def test_cchess_pvmcts_torch_nn_train():
    assert run(['python', '-m', 'board_game.players.pvmcts.cchess_pvmcts_torch_nn_train', '--iteration_num', '2'])
